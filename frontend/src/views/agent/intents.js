...
export class IntentsEdit extends Component {
    ...
    handleStrategy(contentBlock, callback, contentState, item) {
            item.entities.forEach(entity => {
                if (entity.begin !== -1) {
                callback(entity.begin, entity.end);
            }
        });
    }
    ...
    HandleSpan = (props,item,trainingIndex) => {
        ...
        item.entities.map((itemData,prevIndex)=>{
            if(props.decoratedText==itemData.value && props.start == itemData.begin){ //first we find a match from state-entities cause you cannot pass args from startegy to componen
