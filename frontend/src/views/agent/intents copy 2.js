...
class MyCustomBlock extends React.Component {
    ...
    render() {
      return (
        <span className='tag-slot-container'>
          {this.props.children}
        </span>
      );
    }
  ...
  class Mark extends React.Component {
    ...
    render() {
      return <span className="mark"> {this.props.children}</span>
    }
  }
...
export class IntentsEdit extends Component {
                    ...
                    .then((result) => {
                            ...
                            result.training.map((item,index)=>{
                                ...
                                const blocksFromHTML = convertFromHTML(
                                    this.getAnnotatedText(item),
                                    getSafeBodyFromHTML, 
                                    DefaultDraftBlockRenderMap.merge(Map({
                                        'tag': { //change what draftjs will read as block type
                                            //Add New Tags here so draft js understands these tag names  
                                            element: 'tag', //change what it sees in HTML
                                          },
                                    })));
    ...
    editTrainingData = (editorState,setFieldValue,index) => {
        ...
        // const content = this.state.training[index].editorState.getCurrentContent();
        const getText = editorState.getCurrentContent().getPlainText('\u0001');
        
        const newEditor = this.addEmptyBlock(editorState);
