...
export class IntentsComponent implements OnInit {
  ...
  edit(intent) {
    this._router.navigate(["/agent/default/edit-intent", intent.id])
  }

  train(intent) {
    this._router.navigate(["/agent/default/train-intent", intent.id])
  }

  delete(intent) {
    if (confirm('Are u sure want to delete this story?')) {
      this.coreService.displayLoader(true);
      this.intentService.delete_intent(intent.id).then((s: any) => {
        this.ngOnInit();
